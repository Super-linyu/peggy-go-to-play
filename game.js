...
function resizeCanvas() {
    ...
    const containerHeight = window.innerHeight - 200; // 留出空间给UI元素
    ...
    // 更新游戏对象位置（如果游戏已经初始化）
    if (game.isRunning || game.level > 0) {
        updateGamePositions();
    }
}
...
function updateGamePositions() {
    ...
    // 更新房屋位置
    game.house.x = CANVAS_WIDTH - HOUSE_SIZE - 50;
    game.house.y = CANVAS_HEIGHT - HOUSE_SIZE - 100;
    
    // 更新障碍物位置
    for (let obstacle of game.obstacles) {
        obstacle.y = CANVAS_HEIGHT - OBSTACLE_SIZE - 100;
    }
}
